...
<html>
<head>
  <title>Foul Suggestion Admin</title>
...
<script>
  ...
  function displayDate1(sug , q) {
    ...
    window.location.href = 'updateallowsug.php?a='+sug+'&q='+ q;

  }
  function displayDate2(sug , q) {
    alert("Disallowing--> Sug:"+sug +" Admin: " +q);
    window.location.href = 'updatedisallowsug.php?a='+sug+'&q='+ q;
...
<?php
    if(isset($_GET['q'])){
      ...
      $dbname = "wtproject";
      $q=$_GET['q'];
      echo "<br>$q".", Suggestions requiring attention:"."<br>"."<hr>";
    }
?> 
<h3></h3>
      ...
      <?php
        ...
        if ($result->num_rows > 0) {
            ...
            while($row = $result->fetch_assoc() and $i<=10) {
                echo "<tr>";
                echo "<td>" . $i . "</td>";
                echo "<td>" . "<a href=\"queryengine04.php?q=$q&p=$row[sug]\">".$row['sub'] ."</a>" . "</td>";
      ...
      <br>
        <?php
          if(isset($_GET['p'])){
            ...
            $sql = "SELECT sub, descp, tos, sug from suggestions where sug=$sug ";
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
